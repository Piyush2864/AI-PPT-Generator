    ...
    <div className="mx-auto max-w-4xl animate-fade-in">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" className="gap-1.5" onClick={handleDuplicate} isLoading={isDuplicating}>
            <Copy className="h-4 w-4" />
            Duplicate
          </Button>

          {(isCompleted || presentation.status === 'FAILED') && (
            <Button variant="outline" size="sm" className="gap-1.5" onClick={handleRegenerate} isLoading={isRegenerating}>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {[
          ...
          <span key={m.label} className="flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1 text-xs font-medium text-muted-foreground">
            {m.icon}
      ...
      {isCompleted && slides.length > 0 && (
        <div className="mt-4 flex items-center gap-2 rounded-lg border border-primary/20 bg-primary/5 px-4 py-2.5">
          ...
          <p className="text-xs text-primary">
            Click <span className="font-semibold">Edit</span> on any slide to modify, or use{' '}
            <span className="font-semibold">↑ ↓</span> to reorder.
          </p>
        </div>
      )}
